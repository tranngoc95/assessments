...
public class View {
    ...
    public Category getItemCategory(){
        return getItemCategory(true);
    }

    public Category getItemCategory(boolean required) {
        ...
        Integer input = io.readInt(message, 1, index, required);
        if(!required && input==null){
            return null;
        }
        return Category.values()[input - 1];
    }
    ...
    public String getStateAbbr(){
        ...
        do {
            valid=true;
            input = io.readRequiredString("Two-Letter State Abbreviation: ");
            if(input.length()!=2){
                io.println("Wrong format.");
                valid=false;
            }
        } while (!valid);
    ...
    public Forage makeForage(Forager forager, Item item) {
        ...
        forage.setItem(item);
        forage.setDate(io.readLocalDate("Forage date [MM/dd/yyyy]: ", LocalDate.now()));
    ...
    public Forager makeForager(){
        ...
        forager.setState(getStateAbbr());
        return forager;
    }

    public Item updateItem(Item item){
        io.println("Updating item.");
        io.println("Press [Enter] to keep original value.%n");

        Category cate = getItemCategory(false);
        if(cate!=null) {
            item.setCategory(cate);
        }

        String name = io.readString("Item Name: ");
        if(!name.isBlank()) {
            item.setName(name);
        }

        BigDecimal dpk = io.readBigDecimal("$/Kg: ", BigDecimal.ZERO, new BigDecimal("7500.00"), false);
        if(dpk!=null) {
            item.setDollarPerKilogram(dpk);
        }
        return item;
    }
    ...
    public void displayItems(List<Item> items) {
        ...
        for (Item item : items) {
            io.printf("%s: %s, %s, %.2f $/kg%n",
                    item.getId(), item.getName(), item.getCategory(), item.getDollarPerKilogram());
        }
    }
    ...
    public void displayForagers(List<Forager> foragers){
        ...
        foragers.forEach(a->io.printf("%s, %s - %s%n", a.getFirstName(), a.getLastName(), a.getState()));
    }

    public void displayKgPerItem(Map<Item, Double> report){
        if(report==null || report.size()==0){
            io.println("There is no item to display.");
            return;
        }
        report.keySet().stream()
                .sorted(Comparator.comparing(Item::getName))
                .forEach(a->io.printf("%s: %.2fkg%n",a.getName(),report.get(a)));
    }

    public void displayCategoryValue(Map<Category, BigDecimal> report){
        if(report==null || report.size()==0){
            io.println("There is no report to display.");
            return;
        }
        for(Map.Entry<Category, BigDecimal> kv : report.entrySet()){
            io.printf("%s: $%.2f%n", kv.getKey(), kv.getValue().doubleValue());
        }
    }
}
