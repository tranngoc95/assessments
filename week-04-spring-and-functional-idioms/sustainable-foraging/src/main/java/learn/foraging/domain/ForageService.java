...
public class ForageService {
    ...
    public List<Forage> findByForager(Forager forager) throws DataException {
        return forageRepository.findByForager(forager);
    }
    ...
    private void validateDuplicate(Forage forage, Result<Forage> result){
        ...
        List<Forage> forageList = findByDate(forage.getDate());
        if(forageList.stream().anyMatch(a->a.getForager().equals(forage.getForager())
                && a.getItem().equals(forage.getItem()))){
            result.addErrorMessage("Forage can't be duplicated.");
        };
    }
}
